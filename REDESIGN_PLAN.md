...
2026-09-20 사용자 요청으로 이력서를 `/resume/` 하나로 통합했다. 한국어·영어를 별도 이력서 페이지로 관리하지 않고 `src/resume.content.json`의 같은 항목에서 관리하며, 브라우저 언어와 수동 선택에 따라 같은 문서에서 표시한다. 기존 `/kr/`, `/en/`은 호환 리디렉션이다. 같은 문서의 언어 선택 UI는 허용한다. 아래 과거의 세 페이지 독립 지침보다 이 문단이 우선한다. 교육 페이지는 독립 문서로 유지하고 페이지 간 내비게이션은 추가하지 않는다.
